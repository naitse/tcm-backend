...
import com.mulesoft.pusher.util.PusherUtil;
import java.net.URL;
...
public class PusherChannel {
    ...
    public PusherResponse getChannelInfo() throws PusherTransportException{
    	//Build URI path
    	String uriPath = PusherUtil.buildChannelURIPath(this.channelName, this.pusherApplicationId);
    	//Build query
    	String query = PusherUtil.buildQuery(null, null, "", this.pusherApplicationKey);
    	//Generate signature
    	String signature = PusherUtil.buildGetAuthenticationSignature(uriPath, query, this.pusherApplicationSecret);
    	//Build URI
    	URL url = PusherUtil.buildURI(uriPath, query, signature);
